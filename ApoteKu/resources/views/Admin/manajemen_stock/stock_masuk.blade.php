        ...
        <div class="card">
            <div class="container">
                ...
                <button class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#createModal">Tambah
                    Stok Masuk</button>
                <table id="stokmasuk" class="table table-striped dataTable-table">
                    <thead>
                        <tr>
                            <th width=10px>No</th>
                            <th>Nama Staff</th>
                    ...
                    <tbody>
                        @foreach($stokmasuk as $index => $stok_masuk)
                        <tr>
                            <td>{{ $index+1 }}</td>
                            <td>{{$stok_masuk->user->name}}</td>
                            <td>{{$stok_masuk->obat->nama }}</td>
                            <td>{{$stok_masuk->jumlah}}</td>
                            <td>{{$stok_masuk->created_at}}</td>

                            <td>
                                <button class="btn btn-warning btn-edit" data-id="{{ $stok_masuk->id }}">Edit</button>

                                @method('DELETE')
                                <a href="{{ route('stok-masuk.destroy', $stok_masuk->id) }}"
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" name="obat_id" id="obat_id" required>
                            @foreach($obat as $obats)
                                <option value="{{ $obats->id }}">{{ $obats->nama }}| Satuan: {{$obats->satuan}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="satuan" class="form-label">Jumlah Obat</label>
